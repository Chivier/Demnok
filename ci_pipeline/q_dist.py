...
ENC_DIM = 4096
WORLD_SIZE = 4
PASSAGE_FILE = "questions.jsonl"
OUTPUT_INDEX = "question.pkl"
...
def build_shard(rank):
    ...
    with torch.no_grad():
        for entry in tqdm(passages, desc=f"rank {rank}", position=rank):
            ...
            shard_index.append({"qid": entry["qid"], "doc_id": entry["doc_id"], "emb": emb })
    ...
    if rank == 0:
        ...
        for r in range(world_size):
            ...
            shard.extend(shard_index)
            print(f"[rank 0] loaded shard {r} with {len(shard)} entries")
